...
extract_from_file() {
	...
	local project_name=$(basename "$(dirname "$jsonl_file")" | sed 's/-Users-kyle-/~\//' | sed 's/-/\//g')
...
extract_from_all() {
	local output_file="all_user_messages_$(date +%Y%m%d_%H%M%S).txt"
...
extract_from_project() {
	...
	local output_file="${project_name//\//_}_user_messages_$(date +%Y%m%d_%H%M%S).txt"
	local total_messages=0
	local total_sessions=0
	...
	local search_pattern=$(echo "$project_name" | sed 's/~/Users-kyle/' | sed 's/\//\-/g')
	local found_project=""
	...
	if [ -z "$found_project" ]; then
		...
		for project_dir in "$PROJECTS_DIR"/*; do
			if [ -d "$project_dir" ]; then
				local display_name=$(basename "$project_dir" | sed 's/-Users-kyle-/~\//' | sed 's/-/\//g')
				echo "  - $display_name"
			fi
		done
		exit 1
